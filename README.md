...
Riešenie cieli na ~8 h a vedome nerobí veci, ktoré by boli pri tomto zadaní over-engineering (DB, MediatR, cache úložiska, refresh tokeny, multi-user, CI/CD). Sú navrhnuté ako jednoduché rozšírenia — detaily a odôvodnenie v [design dokumente](docs/2026-06-18-sprava-projektov-design.md) (§13–14). **.NET Aspire** bolo zvážené ako voliteľná vrstva navrchu pre orchestráciu a observability, ale vedome vyradené: orchestrácia Blazor WASM klienta je zatiaľ len v preview a vyžadovala by invazívne zmeny klienta bez úmernej hodnoty pri tomto rozsahu. Aplikácia je naň pripravená (Serilog → OTLP), no funguje plne aj bez neho.
